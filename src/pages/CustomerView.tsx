...
const CustomerView: React.FC<CustomerViewProps> = ({ 
  ...
  return (
    <div className={`animate-in fade-in duration-700 ${theme === 'light' ? 'light-mode bg-[#f4f4f5] text-black' : 'dark-mode text-white'}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 mb-8 sticky top-24 z-30 pointer-events-none space-y-4">
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
            <div className="pointer-events-auto relative w-full max-w-md group"><Search className="absolute left-4 top-3.5 h-5 w-5 opacity-40 group-focus-within:text-[#d94e28] transition-colors" /><input type="text" placeholder={t.search} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full backdrop-blur-xl border rounded-full py-3 pl-12 pr-6 placeholder-opacity-50 focus:outline-none shadow-lg ${theme === 'light' ? 'bg-white/80 border-black/10 text-black' : 'bg-[#121212]/90 border-white/10 text-white'}`} /></div>
            <div className="pointer-events-auto relative">
               {!showCallMenu ? <div className={`flex gap-3 p-1 rounded-2xl backdrop-blur-xl border shadow-xl ${theme === 'light' ? 'bg-white/90 border-black/10' : 'bg-[#121212]/90 border-white/10'}`}><button onClick={() => setShowCallMenu(true)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm ${theme === 'light' ? 'bg-black/5 hover:bg-black/10 text-black' : 'bg-white/5 hover:bg-white/10 text-white'}`}><User size={16}/> {t.service}</button><button onClick={onRequestBill} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm ${theme === 'light' ? 'bg-black/5 hover:bg-black/10 text-black' : 'bg-white/5 hover:bg-white/10 text-white'}`}><FileText size={16}/> {t.bill}</button></div> : <div className="flex gap-2 p-2 rounded-2xl bg-[#121212] border border-white/20 shadow-xl animate-in slide-in-from-right">{serviceOptions.map(opt => <button key={opt.id} onClick={() => { onCallWaiter(opt.id); setShowCallMenu(false); }} className="p-3 bg-white/5 hover:bg-[#d94e28] rounded-xl text-white transition-colors" title={opt.label}>{opt.icon}</button>)}<button onClick={() => setShowCallMenu(false)} className="p-3 text-white/40 hover:text-white">X</button></div>}
            </div>
        </div>
        <div className="pointer-events-auto flex flex-wrap justify-center gap-2">
            {DIETARY_FILTERS.map((filter: any) => <button key={filter.id} onClick={() => toggleFilter(filter.id)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${activeFilters.includes(filter.id) ? 'bg-green-500/20 border-green-500 text-green-400' : 'opacity-50 border-transparent hover:opacity-100'}`}><filter.icon size={12}/> {filter.label}</button>)}
            <button onClick={() => setShowMacros(!showMacros)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${showMacros ? 'bg-blue-500/20 border-blue-500 text-blue-400' : 'opacity-50 border-transparent hover:opacity-100'}`}><Dumbbell size={12}/> Macros</button>
        </div>
        {/* Mood Selector */}
        <div className="pointer-events-auto flex justify-center gap-2 overflow-x-auto pb-2">
           {['comfort', 'healthy', 'energizing', 'indulgent', 'quick'].map(mood => (
             <button key={mood} onClick={() => setSelectedMood(selectedMood === mood ? null : mood)} className={`px-3 py-1 rounded-full text-xs uppercase tracking-widest font-bold border transition-all ${selectedMood === mood ? 'bg-purple-500/20 border-purple-500 text-purple-400' : 'opacity-50 border-transparent hover:opacity-100'}`}>{mood}</button>
           ))}
        </div>
      </div>

      <div className="sticky top-[88px] z-20 bg-inherit backdrop-blur-xl border-y border-white/5 py-4 mb-10 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-6 overflow-x-auto scrollbar-hide"><div className="flex gap-4 min-w-max">
          {CATEGORIES.map((cat: any) => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`flex items-center gap-2 px-6 py-3 rounded-full border transition-all duration-300 ${activeCategory === cat.id ? 'bg-[#d94e28] text-white border-[#d94e28] shadow-lg scale-105' : 'bg-transparent opacity-50 hover:opacity-100 border-current'}`}>
              <cat.icon size={16}/>
              <span className="text-sm font-medium tracking-wide">{cat.name}</span>
            </button>
          ))}
          <button onClick={() => setActiveCategory('favorites')} className={`flex items-center gap-2 px-6 py-3 rounded-full border transition-all duration-300 ${activeCategory === 'favorites' ? 'bg-[#d94e28] text-white border-[#d94e28]' : 'bg-transparent opacity-50 hover:opacity-100 border-current'}`}>
            <Heart size={16} fill={activeCategory === 'favorites' ? "currentColor" : "none"} />
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-32">
        {filteredItems.map(item => {
            ...
            return (
              <div key={item.id} onClick={() => setSelectedItem(item)} className={`group cursor-pointer border rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl ${isSoldOut ? 'opacity-75' : ''} ${theme === 'light' ? 'bg-white border-gray-200 hover:border-gray-400' : 'bg-white/5 border-white/5 hover:border-white/20'}`}>
                <div className="relative aspect-[4/3] overflow-hidden">
                  <img src={item.image} alt={item.name} className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${isSoldOut ? 'grayscale contrast-125' : 'grayscale-[20%] group-hover:grayscale-0'}`} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-60" />
                  <button onClick={(e) => toggleFavorite(e, item.id)} className="absolute top-4 right-4 p-2 rounded-full bg-black/40 backdrop-blur-md text-white hover:bg-white hover:text-red-500 transition-all z-10 opacity-0 group-hover:opacity-100"><Heart size={18} fill={favorites.includes(item.id) ? "currentColor" : "none"} className={favorites.includes(item.id) ? "text-red-500" : ""} /></button>
                  {isSoldOut && <div className="absolute inset-0 flex items-center justify-center bg-black/60 backdrop-blur-[2px] z-10"><span className="px-4 py-2 border-2 border-white text-white font-bold uppercase tracking-widest -rotate-12 bg-black/20 backdrop-blur-md">{t.soldOut}</span></div>}
                  {isLowStock && <div className="absolute top-4 left-4 z-10"><span className="px-2 py-1 bg-orange-500 text-white text-[10px] font-bold uppercase rounded-md shadow-lg animate-pulse">Only {stock} Left</span></div>}
                  <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end text-white">
                    {item.tags.length > 0 && <div className="flex gap-2">{item.tags.slice(0, 2).map((tag: string, i: number) => <span key={i} className="px-2 py-1 bg-white/10 backdrop-blur-md rounded-md text-[10px] font-bold uppercase tracking-wider">{tag}</span>)}</div>}
                    <div className="flex flex-col items-end">
                       {happyHour && item.category === 'coffee' && <span className="text-[10px] bg-[#d94e28] text-white px-1 rounded">-20%</span>}
                       <span className="text-2xl font-serif font-medium">{curr.symbol}{(displayPrice * curr.rate).toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="text-xl font-serif mb-2 group-hover:text-[#d94e28] transition-colors">{item.translations?.[language]?.name || item.name}</h3>
                  <p className="opacity-60 text-sm leading-relaxed line-clamp-2">{item.translations?.[language]?.description || item.description}</p>
                  {showMacros && item.macros && (
                     <div className="flex gap-2 mt-3 text-[10px] opacity-60">
                        <span className="bg-current/10 px-2 py-1 rounded">P: {item.macros.protein}</span>
                        <span className="bg-current/10 px-2 py-1 rounded">C: {item.macros.carbs}</span>
                        <span className="bg-current/10 px-2 py-1 rounded">F: {item.macros.fat}</span>
                     </div>
                  )}
                </div>
              </div>
